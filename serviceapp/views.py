...
def delivaryvoyreg(request):
    msg=""
    if request.POST:
        name=request.POST["t1"]
        email=request.POST["t2"]
        phone=request.POST["t3"]
        uname=request.POST["t4"]
        password=request.POST["t5"]
        c.execute("insert into delivaryboy(name,email,phone,uname) values(%s,%s,%s,%s)",[name,email,phone,uname])
        db.commit()
        c.execute("insert into login (uname,password,usertype,status) values (%s,%s,%s,%s)",[uname,password,'delivaryboy','approved'])
        db.commit()
    return render(request,"sc/adddelivaryboy.html")
def login(request):
    msg=""
    if request.POST:
...
def adddelevary(request):
    if request.POST:
        ...
        c.execute("insert into delivaryboy (name,email,phone,uname) values('"+str(name)+"','"+str(email)+"','"+str(phone)+"','"+str(uname)+"')")
        db.commit()
        c.execute("insert into login (uname,password,usertype,status) values('"+str(uname)+"','"+str(password)+"','DelivaryBoy','approved')")
        db.commit()
    return render(request,"adddelevary.html")
def dbase(request):
    
    return render(request,"delivaryboy/dbase.html")
def viewbookingss(request):
    c.execute("select us.name,us.phoneno,us.address,sr.name,sr.company,b.bid,b.status from userreg us join booking b on(b.userid=us.uid) join screg sr on(sr.scid=b.scid) join assigndelivary d on(d.bookingid	=b.bid) group by b.bid")
    data=c.fetchall()
    return render(request,"delivaryboy/viewbookings.html",{"data":data})
def screg(request):
...
def booking(request):
    ...
    if request.POST:
            ...
            pic=request.POST["ra"]
            query="insert into booking(company,bookingdate,problem,status,userid,scid,enddate,picup) values('"+str(company)+"','"+str(bookingdate)+"','"+str(problem)+"','"+str(status)+"','"+str(userid)+"','"+str(idd)+"','"+str(enddate)+"','"+str(pic)+"')"
            c.execute(query)
            db.commit()
...
def assigndelivaryboy(request):
    id=request.GET["id"]
    cid=request.GET["cid"]
    c.execute("insert into  assigndelivary (bookingid,cid,status) values(%s,%s,%s)",[id,cid,'assigned'])
    db.commit()
    return HttpResponseRedirect("/viewbooking/")

def viewbooking(request):

    c.execute("select booking.*,userreg.* from booking join userreg on booking.userid=userreg.uid where scid='"+str(request.session['scid'])+"' ")
    data=c.fetchall()
    print(data)
    if data:
...
def feedback(request):
    ...
    if request.POST: 
        ...
        db.commit()
        msg="Addsuccessfully"
    return render(request,"user/feedback.html",{"msg":msg})
...
def useraddmessage(request):
    ...
    return render(request,"user/useraddmessage.html",{"data1":data1,"msg":msg})
...
def deliverstatus(request):
    ...
    if stat=="Pickup":
        c.execute("update booking set status='Pickup' where bid='"+str(id)+"'")
        db.commit()
        # booking.objects.filter(bid=id).update(status=stat)
    if stat=="Delivered":
        # booking.objects.filter(bid=id).update(status=stat)
        c.execute("update booking set status='Delivered' where bid='"+str(id)+"'")
        db.commit()
    return HttpResponseRedirect('/viewbookings')